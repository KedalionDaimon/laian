...
; In try-hierarchisation, now I inserted:
;            (mutated-knowledge
;              (cond
;                ((null possible-hiers)
;                  (establish-hypothesis
;                    input-list
;                    (cond ((< 2 (/ segment-length 2)) 2)
;                           (t (floor (/ segment-length 2))))
;                    knowledge))
;                (t
...
;                    knowledge)))
;
; - In other words, HYPOTHESES are now created with HALF
; the segment length - which greatly increases the chance of
; re-matching them later (on the FULL segment length). I.e. if
; you create a hypothesis of 4 words, you can much easier "get all"
; words in a range of 8 words. Otherwise, you created a hypothesis
; of 8 words in a range of 8 words - which is more precise, but
; quite unrealistic to match.

; This greatly improved the quality of conversation - here,
; starting from zero:

; (load "laian20170216.lsp")
; 
; (LEARNING ARTIFICIAL INTELLIGENCE APPLYING NOTIONS (LAIAN)) 
...
; 
; (HUMAN------) 
; (I GREET THEE LAIAN ARE YOU OPERATIONAL TODAY Q)
; 
; (MACHINE----) 
; NIL 
; 
; (HUMAN------) 
; (YOUR MIND IS STILL SUNKEN IN THE VOID AND YOU KNOW NOT WHAT TO SAY)
; 
; (MACHINE----) 
; (MY MIND IS STILL SUNKEN IN THE VOID AND ME KNOW NOT WHAT) 
; 
; (HUMAN------) 
; (TO SAY TO SAY TO ME THE SENSIBLE REPLY IS STILL A PUZZLING IDEA TO YOU)
; 
; (MACHINE----) 
; (SAY) 
; 
...
; 
; (HUMAN------) 
; (NOTHING NOTHING IS NOT A VERY GOOD WAY TO REPLY YOU KNOW THAT Q)
; 
; (MACHINE----) 
; (NOT A VERY GOOD WAY TO REPLY ME KNOW THAT Q) 
; 
; (HUMAN------) 
; (YOU JUST LEARNT IT YOU ARE LARGELY STILL IN THE VOID)
; 
; (MACHINE----) 
; (SAY TO YOU THE SENSIBLE REPLY IS STILL A PUZZLING IDEA TO) 
; 
; (HUMAN------) 
...
(defvar *knowledge-areas-count* 2000)
...
(defvar *hierarchisation-sequence*
  '((16 4) (16 3) (16 2) (8 4) (8 2) (8 1) (4 2) (4 1) (4 1)))
...
; NOW THUNK - no greater categories than maximum category size:
(defun all-unions (some-set knowledge)
  (cond ((null knowledge) nil)
        ...
        ((nearly-equal-sets some-set (cadar knowledge))
          (all-unions some-set (cdr knowledge)))
        (t (let ((uni-set (union-set some-set (cadar knowledge))))
          (cond ((< *max-category-size* (length uni-set))
                  (all-unions some-set (cdr knowledge)))
                (t (cons uni-set
                  (all-unions some-set (cdr knowledge)))))))))
